+++
title = 'Siyuan Liu'
date = 2024-09-29T14:27:08+08:00
draft = false
alumni = false

type = 'people'
layout = 'people'

name = 'syliu'
name_cn = '刘思源'
name_en = 'Siyuan Liu'
degree = 'Undergraduate'
image = '/people/syliu/photo.jpg'

email = '18234991499@163.com'
github = 'liusy1499-eng'

# Verification | Simulation | Synthesis
research_label = ["Simulation"]

[[introduction]]
    content = "I am an undergraduate student at Jilin University. I will join Xi'an Jiaotong University (XJTU) as a master student in Fall 2026, advised by Prof. Peng Zhang."

[[introduction]]
    content = "I work on data plane verification, focusing on automated correctness checking of network forwarding behaviors. My research applies symbolic reasoning and decision diagram techniques to model network data planes and verify key properties such as reachability, isolation, and loop-freedom. I am particularly interested in improving the scalability and real-world applicability of verification tools, enabling them to handle large-scale networks with minimal performance overhead."

[[research]]
    interest = 'Network Verification'

[[education]]
    school = "Xi'an Jiaotong University"
    major = "School of Computer Science and Technology"
    degree = 'Master in Computer Science'
    time = '2026.9 - now'
[[education]]
    school = 'JiLin University'
    major = "School of Computer Science and Technology"
    degree = 'B.E. in Computer Science'
    time = '2022.9 - 2026.6'

[[practice]]
    enterprise = 'Yonyou Auto Information Technology (Shanghai) Co., Ltd.'
    work = 'Junior Backend Development Engineer (Intern)'
    time = '2025.9 - 2025.12'
    
[[award]]
    title = "National Endeavor Scholarship, 2022-2024"
+++
